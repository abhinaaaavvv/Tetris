...
public class TetrisPanel extends JPanel {
    ...
    private void tryRotate(boolean cw) {
        ...
        int[][] kicks = new int[][] { { -1, 0 }, { 1, 0 }, { -2, 0 }, { 2, 0 }, { 0, -1 } };
        ...
        // Revert if none fit
        if (cw) current.rotateCCW(); else current.rotateCW();
    }
  ...
  private boolean isLockedAboveTop(Tetromino piece, int gx, int gy) {
    ...
    protected void paintComponent(Graphics g0) {
        ...
        for (int y = 0; y < Board.ROWS; y++) {
            for (int x = 0; x < Board.COLS; x++) {
                ...
                if (ord >= 0) {
                    drawCell(g, bx + x * CELL, by + y * CELL, Tetromino.colorForOrdinal(ord));
                }
            }
        }
    ...
    private void drawPiece(Graphics2D g, Tetromino piece, int gx, int gy, int bx, int by, boolean ghost) {
